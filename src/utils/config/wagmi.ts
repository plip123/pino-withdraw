...
export const getMagicConnectorFn = (id: string, name: string = 'Magic', chainId: number = polygon.id) => {
  const walletDetails = {
    id,
    name,
    rdns: 'Magic',
...
const getMagicConnector = (id: string, name: string = 'Magic', chainId: number = polygon.id) => () => ({
    id,
    name,
    rdns: 'Magic',
    iconUrl: 'https://r2rtoken.com/assets/aiBg-DFJx_raV.png',
  ...
  [
    {
      ...
      wallets: [
        getMagicConnector(`magic-${polygon.id}`, `Magic ${polygon.name}`, polygon.id),
        metaMaskWallet, rabbyWallet, walletConnectWallet,
      ],
    },
  ],
  {
